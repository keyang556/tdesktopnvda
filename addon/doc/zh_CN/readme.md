...
* 改善 Telegram 会话列表和消息列表的 `Tab` 焦点进入与焦点朗读。
* 改善使用电话号码登录时，国家/地区选择列表的朗读。
* 保留 Telegram 原本提供的会话、消息和列表项目名称。
...
Telegram Desktop 6.8.3 以上版本的某些画面可能会让 NVDA 无法朗读当前焦点所在的列表行，或在按 `Tab` 进入列表容器时没有读出当前行。最常见的情况是：您在会话列表或消息列表中移动，但 NVDA 没有读出当前项目。

此附加组件会在 NVDA 端补上受影响 Telegram 列表容器的可聚焦状态，并避开 Telegram 在这些行上造成朗读中断的信息，让 NVDA 可以继续读出当前焦点项目。同样的列表行保护也应用在电话号码登录流程中的国家/地区选择对话框。
...
* Telegram Desktop for Windows 6.8.3 以上版本，特别是具有相同列表焦点或朗读问题的版本。
* NVDA 2024.1 或更新版本。
