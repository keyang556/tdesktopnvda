...
* 改善 Telegram 聊天室清單和訊息清單的 `Tab` 焦點進入與焦點朗讀。
* 改善使用電話號碼登入時，國家/地區選擇清單的朗讀。
* 保留 Telegram 原本提供的聊天室、訊息與清單項目名稱。
...
Telegram Desktop 6.8.3 以上版本的某些畫面可能會讓 NVDA 無法朗讀目前焦點所在的清單列，或在按 `Tab` 進入清單容器時沒有讀出目前列。最常見的情況是：您在聊天室清單或訊息清單中移動，但 NVDA 沒有讀出目前項目。

此附加元件會在 NVDA 端補上受影響 Telegram 清單容器的可聚焦狀態，並避開 Telegram 在這些列上造成朗讀中斷的資訊，讓 NVDA 可以繼續讀出目前焦點項目。相同的清單列保護也套用在電話號碼登入流程中的國家/地區選擇對話框。
...
* Telegram Desktop for Windows 6.8.3 以上版本，特別是具有相同清單焦點或朗讀問題的版本。
* NVDA 2024.1 或更新版本。
